        ...
        <div class="loginSec">
            ...
            <div class="loginFormBox">
                <div class="d-flex justify-content-between">
                    <h3>Verify Mobile</h3>
                    <a href="{{ route('register', ['data' => $data]) }}" class="backBtn"><img
                            src="{{ asset('frontend/images/backbtn.svg') }}" alt="" width="14"> Back</a>
                </div>
                <p class=" mb-4">Verify your phone number to finish setting up your account</p>
                <form method="post" action="{{ route('register.otp.check') }}">
                    @csrf
                    <input type="hidden" name="userForm" value="{{ $userForm }}">
                    <div class="mb-4">
                        <label class="form-label">Mobile Number</label>
                        <input class="form-control w-100 @error('id') is-invalid @enderror" type="text" readonly
                            placeholder="" name="id" id="id" value="{{ $data }}" required
                            autocomplete="id" autofocus>
                    </div>
                    <a href="{{ route('register') }}" class="textUnderline mb-3">Change Mobile Number</a>
                        ...
                        <div class="otpFind">
                            ...
                            @for ($i = 0; $i < 6; $i++)
                                <input type="text" class="form-control otp-input" maxlength="1" name="otp[]"
                                    value="{{ isset($otpDigits[$i]) ? $otpDigits[$i] : '' }}" required oninput="handleInput(this, {{ $i }})" autocomplete="off" />
                            @endfor
                        </div>
                        @if (isset($error))
            ...
            <div class="modal-content">
                ...
                <div class="modal-body pt-0">
                    <form>
                        ...
                        <p class="text-center">Your mobile no successfully verified</p>
                        <div class="text-center my-2 mt-4">
                            <button type="button" class="btn btn-primary-gradient fs-7 rounded-2 w-75">Continue
                                Registration</button>
                        </div>
